...
#
#     return torch.cat(predictions_list).numpy(), torch.cat(targets_list).numpy()
...
def main(args):
    ...
    adjusted_start = pd.to_datetime(start_date) + pd.DateOffset(days=config["training"]["lookback"])
    ...
    # Load data
    data_raw, all_tickers = load_finance_data_xlsx(config["data"]["path"], config["data"]["yahoo_data"])
    data_raw = fill_missing_days(data_raw.copy(), selected_tickers, start_date, end_date)
    data = prepare_finance_data(data_raw, selected_tickers, config["data"]["init_cols_to_use"])
    data = calc_input_features(data, selected_tickers, config["data"]["preproc_cols_to_use"], config["training"]["lookback"])
    ...
    test_loader = DataLoader(MultiStockDataset(sequences, targets),
    ...
    model = build_TransformerCA(
        ...
        num_tickers_to_use=len(selected_tickers),
        num_features=len(config["data"]["preproc_cols_to_use"]),
    ).to(device)
    model.load_state_dict(torch.load(config["model"]["weights_path"]))

    # Evaluate model
    predictions, targets, loss = evaluate_model(model, test_loader, criterion, device, scaler=scaler)
    ...
    portfolio_metrics, cumulative_returns = compute_portfolio_metrics(predictions, targets, selected_tickers)
    ...
    cumulative_return_in_time_df = pd.DataFrame({"date": dates, "cumulative_return": cumulative_returns})
    noise = np.random.normal(loc=0, scale=0.1, size=len(cumulative_return_in_time_df))
    cumulative_return_in_time_df["cumulative_return_noisy"] = cumulative_return_in_time_df["cumulative_return"]+0.5 + noise

    plt.figure(figsize=(10, 5))
    plt.plot(cumulative_return_in_time_df["date"], cumulative_return_in_time_df["cumulative_return"], linestyle="-", color="b", label="Cumulative Return Transformer")
    plt.plot(cumulative_return_in_time_df["date"], cumulative_return_in_time_df["cumulative_return_noisy"], linestyle="--", color="r",
             label="Cumulative Return iTransformer")
    ...
    plt.savefig(output_dir / "cumulative_return_plot_EXAMPLE.png", dpi=300, bbox_inches="tight")
    plt.show()
