...
# 1. Generowanie sztucznych szeregów czasowych z wieloma cechami
def generate_multivariate_data(n_samples=200, num_features=3, noise=0.1):
    x = np.linspace(0, 4 * np.pi, n_samples)
    data = []
    for i in range(num_features):
        y = np.sin(x + i) + noise * np.random.randn(n_samples)  # Dodanie przesunięcia fazy dla różnic
        data.append(y)
    return np.stack(data, axis=1)  # Macierz [n_samples, num_features]
...
def create_sequences(data, lookback):
    ...
    for i in range(len(data
        ...
        targets.append(data[i + lookback, 0])  # Prognozowanie tylko pierwszej cechy
...
data_scaled = np.column_stack([scaler.fit_transform(data[:, i].reshape(-1, 1)).flatten() for i, scaler in enumerate(scalers)])
...
dim_feedforward = 128
dropout = 0.05
model = TransformerModel(input_dim=input_dim, d_model=d_model, nhead=nhead, num_encoder_layers=num_encoder_layers,dim_feedforward=dim_feedforward, dropout=dropout).to(device)
